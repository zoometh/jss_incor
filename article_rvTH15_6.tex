...
\caption{\label{fig:figs}Concatenate of 'a', 'r' and 't' graphical units is 'art'}\label{fig:unnamed-chunk-1}
\end{figure}
...
\caption{\label{fig:figs}Potential spatial relations between graphical units}\label{fig:unnamed-chunk-2}
\end{figure}
...
\begin{figure}[H] 
\centering
\includegraphics[scale=0.05]{gis_1}
...
\begin{figure}[H] 
\centering
\includegraphics[scale=0.05]{gis_2}
...
R> # nodes <- read.table(system.file("extdata", "nodes.tsv", package = "decorr"),
R> #                     sep="\t",stringsAsFactors = FALSE)
R> # edges <- read.table(system.file("extdata", "edges.tsv", package = "decorr"),
R> #                     sep="\t",stringsAsFactors = FALSE)
...
+                       caption="Sixth node of the Torrejon Rubio 1 decoration",
+                       label="nd",
+                       size=7),
+        table.placement="H")
\end{verbatim}
...
\begin{tabular}{rllrlrrrr}
  \hline
 & site & decor & id & type & x & y & coords.x1 & coords.x2 \\ 
  \hline
13 & Torrejon Rubio & Torrejon Rubio 1 &   6 & epee & 366.70 & -563.14 & 366.70 & -563.14 \\ 
   \hline
\end{tabular}
\caption{Sixth node of the Torrejon Rubio 1 decoration} 
\label{nd}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in unlist(listg, recursive = FALSE): object 'g.compar' not found}}\begin{verbatim}
R> plot(image_trim(image_read(eds_compar)))
\end{verbatim}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in image\_read(eds\_compar): object 'eds\_compar' not found}}\end{kframe}
...
\caption{\label{fig:figs}Decoration comparisons between 3 and 4}\label{fig:unnamed-chunk-12}
\end{figure}
