...
\\
\\
In this article we present the \proglang{R} package \pkg{decorr}. Its purpose is to formalise a method based on relative neighborhood graphs (RNG) to analyse any graphical content (here: decorations). As any formal system, iconography can be modelled as spatial features related one with the other depending on rules of spatial proximities; as state by the First Law of Geography: "\emph{everything is related to everything else, but near things are more related than distant things}" \citep{Tobler70}. And, because any graphical system is composed by features (mostly shapes) displayed spatialy, these systems can be represented by features connected (or not) to each other depending on their spatial proximity. To map and analyse these proximities, the model uses concepts and methods coming from Graph Theory, GIS and binary topological relationships (\emph{birel}) modeling. This package has been grounded on the seminal work of \citet{Alexander08} and its first IT implementation by \citet{Huet18a}. 
...
% \end{itemize}
By convention, two contemporaneous GUs having a Voronoi cell sharing a border, have a common edge tagged \code{'='}. This proximity is represented with a plain line. The textual notation of such an edge is \code{'-=-'}. For example: \code{1 -=- 4} means that the nodes \code{1} and \code{4} have a common border.  
\smallbreak
...
The inventory of decorations is stored in the \code{imgs} dataframe. The field \code{imgs$idf} is the short name of the decoration, useful during statistical analysis. The primary key of each decoration is the concatenate of \code{imgs$site} and \code{imgs$decor}. These keys will allow joints with the other dataframes (\code{nodes} and \code{edges})\\
%
% latex table generated in R 3.6.1 by xtable 1.8-4 package
% Fri Dec 04 10:36:29 2020
\begin{table}[H]
\centering
...
It contains the required minimum variables for the analysis (Fig. \ref{fig:tnds}).
...
The \code{edges} dataframe is quite similar to the \code{nodes} dataframe (Fig. \ref{fig:teds}).
...
\begin{tabular}{rllrrl}
 ...
 & site & decor & a & b & type \\ 
  \hline
1 & Cerro Muriano & Cerro Muriano 1 &   1 &   4 & = \\ 
  2 & Cerro Muriano & Cerro Muriano 1 &   1 &   5 & = \\ 
  3 & Cerro Muriano & Cerro Muriano 1 &   3 &   5 & = \\ 
  4 & Cerro Muriano & Cerro Muriano 1 &   1 &   2 & + \\ 
  5 & Cerro Muriano & Cerro Muriano 1 &   1 &   7 & + \\ 
  6 & Cerro Muriano & Cerro Muriano 1 &   3 &   1 & = \\ 
   \hline
\end{tabular}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in list\_dec(imgs, nodes, edges, var = "{}type"{}): unused argument (var = "{}type"{})}}\begin{verbatim}
R> plot(lgrph[[1]],
+       vertex.color = "orange",
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in plot(lgrph[[1]], vertex.color = "{}orange"{}, vertex.frame.color = "{}orange"{}, : object 'lgrph' not found}}\end{kframe}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in plot\_dec\_grph(nodes = nds.df, edges = eds.df, site = sit, decor = dec, : unused argument (lbl.txt = "{}type"{})}}\begin{verbatim}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in plot\_compar(g.compar, c(1, 4), focus = "{}edges"{}, doss = system.file("{}extdata"{}, : object 'g.compar' not found}}\begin{verbatim}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in same\_neds(lgrph, "{}type"{}, "{}edges"{}): could not find function "{}same\_neds"{}}}\begin{verbatim}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in xtable::xtable(df.same\_edges, caption = caption, label = "{}Test\_table\_2"{}, : object 'df.same\_edges' not found}}\end{kframe}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in same\_neds(lgrph, "{}type"{}, "{}edges"{}): could not find function "{}same\_neds"{}}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in same\_neds(lgrph, "{}type"{}, "{}nodes"{}): could not find function "{}same\_neds"{}}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in xtable(df.same\_nodes, digits = c(0)): object 'df.same\_nodes' not found}}\end{kframe}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in xtable(df.same\_edges, digits = c(0)): object 'df.same\_edges' not found}}\end{kframe}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in as.matrix(df.same\_nodes): object 'df.same\_nodes' not found}}\begin{verbatim}
R> dist.edges  <- dist(inv(as.matrix(df.same_edges)))
\end{verbatim}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in as.matrix(df.same\_edges): object 'df.same\_edges' not found}}\begin{verbatim}
R> plot(hclust(dist.nodes), hang = -1, main = "common nodes")
\end{verbatim}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in hclust(dist.nodes): object 'dist.nodes' not found}}\begin{verbatim}
R> plot(hclust(dist.edges), hang = -1, main = "common edges")
\end{verbatim}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in hclust(dist.edges): object 'dist.edges' not found}}\end{kframe}
\end{figure}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in plot\_nds\_compar(g.compar, c(3, 4), doss = system.file("{}extdata"{}, : could not find function "{}plot\_nds\_compar"{}}}\begin{verbatim}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in plot\_eds\_compar(g.compar, c(3, 4), doss = system.file("{}extdata"{}, : could not find function "{}plot\_eds\_compar"{}}}\begin{verbatim}
